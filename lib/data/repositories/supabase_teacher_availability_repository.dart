...
class SupabaseTeacherAvailabilityRepository implements TeacherAvailabilityRepository {
  ...
  Future<void> deletePeriod(String periodId) async {
    await _svc.client.from('teacher_availability').delete().eq('id', int.parse(periodId));
  }

  @override
  Future<void> upsertPeriods(String teacherId, List<AvailabilityPeriod> periods) async {
    final tid = int.parse(teacherId);
    // Strategy: remove existing rows for teacher, then insert provided periods
    await _svc.client.from('teacher_availability').delete().eq('teacher_id', tid);
...
(
  ...
  final body = cleaned.substring(1, cleaned.length - 1); // drop [ and ) / ] and )
